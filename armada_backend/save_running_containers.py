    ...
    try:
        ...
        for container_id in containers_ids:
            try:
                ...
                errors_count += 1
                logging.error('ERROR on getting container parameters for {container_id}:'.format(**locals()))
                traceback.print_exc()
        ...
        if containers_parameters_list or not errors_count:
            ...
            logging.info('Containers have been saved to {saved_containers_path}.'.format(**locals()))
            try:
                _save_containers_parameters_list_in_kv_store(containers_parameters_list)
                logging.info('Containers have been saved to kv store.')
            except:
                traceback.print_exc()
        else:
            logging.info('Aborted saving container because of errors.')
    except:
        traceback.print_exc()
        sys.exit(1)
